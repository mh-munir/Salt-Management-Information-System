...
import DashboardDeferredSectionSkeleton from "@/app/(dashboard)/dashboard/DashboardDeferredSectionSkeleton";
...
const DashboardChartsSection = dynamic(
  () => import("@/app/(dashboard)/dashboard/DashboardChartsSection"),
  { loading: () => <DashboardDeferredSectionSkeleton /> }
);

const DashboardContactStreamSection = dynamic(
  () => import("@/app/(dashboard)/dashboard/DashboardContactStreamSection"),
  { loading: () => <DashboardDeferredSectionSkeleton /> }
);
        ...
        <section className="space-y-4">
          <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
            <DashboardChartsSection
              ...
              totalBusinessInvestment={totalBusinessInvestment}
              totalBusinessReturn={totalBusinessReturn}
              profitPieRingSegments={profitPieRingSegments}
              investmentMixRows={investmentMixRows}
              returnMixRows={returnMixRows}
              formatFullCurrency={formatFullCurrency}
              formatPercent={formatPercent}
            />

            <DashboardContactStreamSection
              language={language}
              contactStream={contactStream}
              contactDirectoryLabel={translate(language, "contactDirectory")}
              liveContactStreamLabel={translate(language, "liveContactStream")}
              noContactsFoundLabel={translate(language, "noContactsFound")}
              viewLabel={translate(language, "view")}
            />
          </div>
        </section>
      </div>
