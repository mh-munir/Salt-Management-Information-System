...
export default function StockPage() {
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      const stockData = (await parseJson(stockRes)) as StockData | null;
      const txData = await parseJson(txRes);

      setStock(stockData);
      setTransactions(Array.isArray(txData) ? (txData as TransactionItem[]) : []);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-slate-500">{translate(language, "loadingStockDashboard")}</p>
      </div>
    );
  }
  ...
  const totalSaleStockKg = getSafeNumber(stock.totalSaleStock ?? stock.totalSold);
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <p className="rounded-md bg-slate-50 px-4 py-3 text-lg text-slate-700">
